...
public class AlgoSimulator {
	...
	public static void main(String[] args) {
		...
		JPanel timerP = new JPanel(new GridBagLayout()); // initialize panel for all buttons		
                ...
                if (selected) {
                	for (int i = 19; i >= 0; i--) {
            			for (int j = 0; j < 15; j++) {
            				final int i2 = i;
            				final int j2 = j;
            				if(map.grids[i][j].getBackground() == UNEXPLORED || map.grids[i][j].getBackground().equals(KNOWN_BLOCK) || map.grids[i][j].getBackground() == UNKNOWN_BLOCK) {
		...
		mapSwitchPanel.add(rTimeMap ,mapSwitchPanelC);
		
		mapP.add(legendPanel);
		mapP.add(mapSwitchPanel);
